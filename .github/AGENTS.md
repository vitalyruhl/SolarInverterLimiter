...
- Do not assume a web UI, examples tree, database, Excel export, standalone
  backend service, or `pyproject.toml` release flow unless the repository
  explicitly introduces them.

## Agent Routing

- `.github/AGENTS.md` is the canonical source for repository-wide rules.
- `.github/agents/control-plane.agent.md` only routes work to the correct
  agent.
...
- If a concrete GitHub Project is configured later, workflow rules may use it
  for issue, PR, and project status coordination.
...
- Require an appropriate project version bump for dependency updates,
  PlatformIO configuration changes, firmware code changes, or build metadata
  changes unless the user explicitly says not to bump.
...
- Use minor version bumps for new public features, new public APIs, and
...
- Bump the repository's main firmware or project version source of truth. Do not
  automatically bump unrelated examples or support-tool versions unless they are
...
- After renaming, rerun rg to ensure old names do not remain in relevant
  project files such as `src/`, `include/`, `lib/`, `test/`, and `docs/`.
