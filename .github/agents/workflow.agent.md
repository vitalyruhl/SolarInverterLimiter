...
- Examples:
  ...
  - `use workflow.begin [update ota docs]` may derive
    `feature/update-ota-docs`.
...
- Dependency updates, PlatformIO configuration changes, firmware code changes,
  and build metadata changes that affect build outputs require an appropriate
  project version bump unless the user explicitly says not to bump.
...
- Use minor bumps for new public features, new public APIs, and compatible
  behavior additions.
