...
<head>
    ...
    <style>
        ...
        .setting { margin: 10px 0; display: flex; align-items: center; }
        label { flex: 1; margin-right: 15px; }
        input, select { 
            flex: 2; 
            padding: 8px; 
        ...
        #status { margin: 15px 0; padding: 10px; display: none; }
    </style>
</head>
<body>
    ...
    <script>
        async function loadSettings() {
            try {
                ...
                showStatus('Error loading settings: ' + error.message, 'red');
            }
        }
            ...
            try {
                ...
                document.querySelectorAll('input').forEach(input => {
                    ...
                    if (input.type === 'checkbox') {
                        ...
                        formData[category][key] = input.type === 'number' ? 
